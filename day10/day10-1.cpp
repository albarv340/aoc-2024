...
std::set<std::pair<int, int>> get_score(std::vector<std::vector<int>> const &grid, int x, int y)
{
    ...
    {
        auto other = get_score(grid, x - 1, y);
        ends.insert(other.begin(), other.end());
    }
    if (y > 0 && grid[y - 1][x] == grid[y][x] + 1)
    {
        auto other = get_score(grid, x, y - 1);
        ends.insert(other.begin(), other.end());
    }
...
int main()
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            if (grid[y][x] != 0)
                continue;
            int score = get_score(grid, x, y).size();
            sum += score;
        }
    }
