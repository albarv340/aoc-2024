...
double get_distance(pair<int, int> const &p1, pair<int, int> const &p2)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            for (const auto &pair : antenna_locations)
            {
                ...
                {
                    auto first_dist = get_distance({x, y}, location1);
                    for (const auto &location2 : values)
                    {
                        ...
                        if (!are_collinear({x, y}, location1, location2))
                            continue;
                        auto second_dist = get_distance({x, y}, location2);
                        if (first_dist == 2 * second_dist || second_dist == 2 * first_dist)
                        {
